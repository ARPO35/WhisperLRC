...
- `role=user`：仅放输入内容，格式为 `翻译：{input}`（`{input}` 包含需翻译句子与上下文 JSON）。
...
- 每个翻译分组最多允许 5 次工具调用；同一 `index` 只允许重听 1 次，超过后会强制收束并要求模型直接输出最终 JSON。
- 若上游模型接口不支持 `tools` 字段，程序会自动降级为普通翻译请求继续处理。
- 翻译流程会先尝试“无工具直出”；仅当直出失败时才进入工具会话并保留多轮上下文。
- 工具会话中不再追加 `role=user` 摘要；每次工具调用后由 `role=assistant` 发送完整对话上下文。
- 当某轮不再调用工具并返回最终答案后，会清空该轮工具会话上下文。
